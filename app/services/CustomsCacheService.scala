...
class CustomsCacheService @Inject()(caching: CustomsHttpCaching, applicationCrypto: ApplicationCrypto)
  ...
  def get(cacheName: String, eori: String)(implicit hc: HeaderCarrier,
                                           ec: ExecutionContext): Future[Option[Map[String, String]]] =
    fetchAndGetEntry[Map[String, String]](cacheName, eori)

  def put(cacheName: String, eori: String, data: Map[String, String])(implicit hc: HeaderCarrier,
                                                                      ec: ExecutionContext): Future[CacheMap] =
    cache(cacheName, eori, data)

  def getByKey[T](cacheId: EORI, key: CacheKey[T])(implicit hc: HeaderCarrier,
                                                   rds: Reads[T],
                                                   executionContext: ExecutionContext): Future[Option[T]] =
    fetchAndGetEntry[T](cacheId.value, key.key)

  def upsert[T](cacheId: EORI, key: CacheKey[T])(
      insert: () => T,
      update: T => T
  ...
  def insert[T](cacheId: EORI, key: CacheKey[T], data: T)(implicit hc: HeaderCarrier,
                                                          wts: Writes[T],
