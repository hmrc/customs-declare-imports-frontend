...
class ObligationGuaranteeController @Inject()(actions: Actions, cache: CustomsCacheService)(
  ...
  def display(): Action[AnyContent] = actions.auth.async { implicit req =>
  ...
  def submit(): Action[AnyContent] = (actions.switch(Feature.submit) andThen actions.auth).async { implicit req =>
    val optionSelected = req.body.asFormUrlEncoded.get("submit").headOption
    optionSelected match {
      case Some("Add") =>
        obligationGuaranteesForm
          .bindFromRequest()
          .fold(
              ...
              if (form.accessCode.isDefined || form.id.isDefined || form.amount.isDefined || form.referenceId.isDefined ||
                ...
                cache.fetchAndGetEntry[ObligationGuaranteeForm](req.user.eori.get, guaranteeTypeFormKey).flatMap {
                  case Some(cached) =>
                    ...
                    cache
                      .cache[ObligationGuaranteeForm](req.user.eori.get, guaranteeTypeFormKey, updatedGuarantees)
                      .map { _ =>
                        Ok(views.html.obligation_guarantee(obligationGuaranteesForm, updatedGuarantees.guarantees))
                      }
                  case _ =>
                    cache
                      .cache[ObligationGuaranteeForm](req.user.eori.get,
                                                      guaranteeTypeFormKey,
                                                      ObligationGuaranteeForm(Seq(form)))
                      .map { _ =>
      ...
      case _ =>
        Logger.error("wrong selection => " + optionSelected.get)
