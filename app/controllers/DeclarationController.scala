...
class DeclarationController @Inject()(
    actions: Actions,
    client: CustomsDeclarationsConnector,
  ...
  def displaySubmitForm(name: String): Action[AnyContent] =
    (actions.switch(Feature.submit) andThen actions.auth).async { implicit req =>
  ...
  def displaySubmitConfirmation(conversationId: String): Action[AnyContent] =
    (actions.switch(Feature.submit) andThen actions.auth).async { implicit req =>
      submissionRepository.getByConversationId(conversationId).map {
        ...
        case None =>
          NotFound(
            views.html.error_template("Submission Not Found",
                                      "Submission Not Found",
                                      "We're sorry but we couldn't find that submission.")
          ) // TODO throw specific ApplicationException type to be handled via ErrorHandler
      }
    }

  def handleSubmitForm(name: String): Action[AnyContent] = (actions.switch(Feature.submit) andThen actions.auth).async {
    implicit req =>
  ...
  def onSubmitComplete: Action[AnyContent] = (actions.switch(Feature.submit) andThen actions.auth).async {
    implicit req =>
      val data: Map[String, String]   = formDataAsMap()
      ...
      if (errors.isEmpty) {
        cacheSubmission(data ++ Map("force-last" -> "false")) { (merged, _) =>
          val props = merged.filterNot(
            entry => navigationKeys.contains(entry._1) || knownBad.contains(entry._1) || entry._2.trim.isEmpty
          )
          updateMetaData(MetaData.fromProperties(props)).flatMap({ maybeMetaData =>
            maybeMetaData match {
              case Some(metadata) => {
                metadata.declaration
                  .flatMap(declaration => declaration.functionalReferenceId)
                  .fold(Future.successful(InternalServerError("Lrn Is Required"))) { localReferenceNumber =>
  ...
  private def invalid(name: String, data: Map[String, String])(
      implicit req: AuthenticatedRequest[AnyContent],
      errors: Map[String, Seq[ValidationError]]
  ): Future[Result] = Future.successful(BadRequest(views.html.generic_view(name, data)))

  private def cacheSubmission(data: Map[String, String])(f: (Map[String, String], CacheMap) => Future[Result])(
      implicit req: AuthenticatedRequest[AnyContent]
  ): Future[Result] = cache.get(appConfig.submissionCacheId, req.user.requiredEori).flatMap { existing =>
  ...
  private def validate(data: Map[String, String]): Map[String, Seq[ValidationError]] =
    data
      .filter(entry => Fields.definitions.keySet.contains(entry._1))
      .map { field =>
        ...
        val fieldErrors: Seq[ValidationError] = failures.map(
          err => ValidationError(errorMessageKey(Fields.definitions(field._1), err), Fields.definitions(field._1))
        )
        field._1 -> fieldErrors
      }
      .filterNot(_._2.isEmpty)

  private def errorMessageKey(field: FieldDefinition, result: ValidationResult): String =
    messagesApi(Seq(s"${field.labelKey}.${result.defaultErrorKey}", result.defaultErrorKey), result.args: _*)

  private def formDataAsMap()(implicit req: Request[AnyContent]): Map[String, String] =
  ...
  private def updateMetaData(metaData: MetaData)(implicit req: AuthenticatedRequest[AnyContent], hc: HeaderCarrier) =
    cache
      .fetchAndGetEntry[ObligationGuaranteeForm](req.user.eori.get, "ObligationGuarantees")
      .flatMap(
        obligationGuaranteeForm =>
          cache
            .fetchAndGetEntry[List[domain.GovernmentAgencyGoodsItem]](req.user.eori.get,
                                                                      GOV_AGENCY_GOODS_ITEMS_LIST_CACHE_KEY)
            .map(_.map { items =>
              val decGoodsItems: Seq[GovernmentAgencyGoodsItem] =
                items.map(
                  rec =>
                    GovernmentAgencyGoodsItem(
                      customsValueAmount = rec.goodsItemValue.get.customsValueAmount,
              ...
              val goodsShipmentNew = metaData.declaration
                .flatMap(_.goodsShipment.map(rec => rec.copy(governmentAgencyGoodsItems = decGoodsItems)))

              metaData.copy(
                declaration = metaData.declaration.map(
                  _.copy(goodsShipment = goodsShipmentNew,
                         obligationGuarantees = obligationGuaranteeForm.get.guarantees)
                )
              )
            })
      )
