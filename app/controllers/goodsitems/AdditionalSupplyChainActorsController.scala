...
class AdditionalSupplyChainActorsController @Inject()(actions: Actions, cacheService: CustomsCacheService)
  ...
  def view(form: Form[_], addSupplyChainActors: Seq[RoleBasedParty])(implicit r: Request[_]): Html =
    role_based_party(form, addSupplyChainActors, messageKeyPrefix,
  ...
  def onPageLoad: Action[AnyContent] = (actions.auth andThen actions.eori).async {
    implicit req =>
      cacheService.getByKey(req.eori, CacheKey.goodsItem).map { goodsItem =>
        Ok(view(roleBasedPartiesForm, goodsItem.map(_.aeoMutualRecognitionParties).getOrElse(Seq.empty)))
      }
  }
      ...
      roleBasedPartiesForm.bindFromRequest().fold(
        (formWithErrors: Form[RoleBasedParty]) =>
          cacheService.getByKey(request.eori, CacheKey.goodsItem).map(goodsItem =>
            BadRequest(view(formWithErrors, goodsItem.map(_.aeoMutualRecognitionParties).getOrElse(Seq.empty)))),
        form =>
