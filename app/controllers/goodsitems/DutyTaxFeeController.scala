  ...
  def onPageLoad: Action[AnyContent] = (actions.auth andThen actions.eori).async {
    implicit req =>
      cacheService.getByKey(req.eori, CacheKey.goodsItem).map { goodsItem =>
        Ok(views.html.goods_items_dutytaxfee(dutyTaxFeeForm, goodsItem.flatMap(_.commodity.map(_.dutyTaxFees)).getOrElse(Seq.empty)))
      }
  }

  def onSubmit: Action[AnyContent] = (actions.auth andThen actions.eori).async {
    implicit request =>
      dutyTaxFeeForm.bindFromRequest().fold(
        (formWithErrors: Form[DutyTaxFee]) =>
          cacheService.getByKey(request.eori, CacheKey.goodsItem).map(goodsItem =>
            BadRequest(views.html.goods_items_dutytaxfee(formWithErrors, goodsItem.flatMap(_.commodity.map(_.dutyTaxFees)).getOrElse(Seq.empty)))),
        form =>
