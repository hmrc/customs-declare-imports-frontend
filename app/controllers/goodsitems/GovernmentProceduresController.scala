...
class GovernmentProceduresController @Inject()(actions: Actions, cacheService: CustomsCacheService)(
  ...
  def onPageLoad: Action[AnyContent] = (actions.auth andThen actions.eori).async { implicit req =>
    cacheService
      .getByKey(req.eori, CacheKey.goodsItem)
      .map(
        goodsItem =>
          Ok(
            views.html.goods_items_government_procedures(governmentProcedureForm,
        ...
        form =>
          cacheService.getByKey(request.eori, CacheKey.goodsItem).flatMap { res =>
            ...
            cacheService
              ...
              .map { _ =>
                Redirect(routes.GovernmentProceduresController.onPageLoad())
              }
        }
      )
  }
