...
class ConsignorController @Inject()(actions: Actions, cache: CustomsCacheService)
  ...
  def onPageLoad: Action[AnyContent] = (actions.auth andThen actions.eori andThen actions.goodsItem) {
    implicit request =>
      ...
      val popForm = request.goodsItem.consignor.fold(form)(form.fill)
      Ok(goods_item_consignor(popForm))
  }
  ...
  def onSubmit: Action[AnyContent] = (actions.auth andThen actions.eori andThen actions.goodsItem).async {
    implicit request =>
      ...
      form.bindFromRequest().fold(
        errors    => Future.successful(BadRequest(goods_item_consignor(errors))),
        consignor =>
          cache.insert(request.eori, CacheKey.goodsItem, request.goodsItem.copy(consignor = Some(consignor))).map { _ =>
