...
class GoodsItemsSellerDetailsController @Inject()(actions: Actions, cacheService: CustomsCacheService)
  ...
  def onPageLoad: Action[AnyContent] = (actions.auth andThen actions.eori andThen actions.goodsItem) { implicit req =>
    ...
    Ok(goods_items_seller_details(popForm))
  }
    ...
    implicit request =>
      form.bindFromRequest().fold(
        (formWithErrors: Form[ImportExportParty]) =>
          Future.successful(BadRequest(goods_items_seller_details(formWithErrors))),
        sellerDetails => {
          ...
          cacheService.insert(request.eori, CacheKey.goodsItem, updatedGoodsItem).map { _ =>
            Redirect(controllers.goodsitems.routes.GoodsItemsBuyerDetailsController.onPageLoad())
          }
        })
  }
}
