...
class CustomsCacheServiceSpec extends CustomsSpec with AuthenticationBehaviours {
  ...
  class CacheScenario[E](cacheSource: String = appConfig.keyStoreSource,
                         cacheName: String = appConfig.submissionCacheId,
                         eori: String = randomUser.requiredEori,
                         cachedData: Map[String, String] = Map.empty) {

    var putData: Option[Map[String, Map[String, Map[String, Map[String, String]]]]] = None

    val service = new CustomsCacheService(
      new CustomsHttpCaching(appConfig, component[HttpClient]) {
        override def fetchAndGetEntry[T](source: String, cacheId: String, key: String)(
            implicit hc: HeaderCarrier,
            rds: Reads[T],
            executionContext: ExecutionContext
        ): Future[Option[T]] = (source, cacheId, key) match {
          case legit if (source == cacheSource && cacheId == cacheName && key == eori) =>
            Future.successful(Some(Protected(cachedData).asInstanceOf[T]))
        ...
        override def cache[A](source: String, cacheId: String, formId: String, body: A)(
            ...
            hc: HeaderCarrier,
            executionContext: ExecutionContext
        ): Future[CacheMap] = (source, cacheId, formId) match {
          case legit if (source == cacheSource && cacheId == cacheName && formId == eori) => {
            putData = Some(
              Map(
                source -> Map(
                  cacheId -> Map(formId -> body.asInstanceOf[Protected[Map[String, String]]].decryptedValue)
                )
              )
            )
  ...
  "put" should {
    ...
    "place given data in named cache for identified user" in new CacheScenario[Map[String, String]]() {
      whenReady(service.put(appConfig.submissionCacheId, randomUser.requiredEori, Map(aKey -> aValue))) { _ =>
        putData.get(appConfig.keyStoreSource)(appConfig.submissionCacheId)(randomUser.requiredEori)(aKey) must be(
          aValue
        )
      }
    }
