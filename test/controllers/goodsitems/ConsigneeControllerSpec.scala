...
class ConsigneeControllerSpec extends CustomsSpec
  with PropertyChecks
  with Generators
  with Lenses
  ...
  def controller(user: SignedInUser, item: Option[GoodsItem]) =
    new ConsigneeController(new FakeActions(Some(user), item), mockCustomsCacheService)

  def view(form: Form[_] = form): String =
    goods_item_consignee(form)(fakeRequest, messages, appConfig).body

  val uri = "/submit-declaration-goods/consignee"
  ...
  "onPageLoad" should {
    ...
    "return BAD_REQUEST" when {
      ...
      "there is no goods items in the cache" in {
  ...
  "onSubmit" should {
    ...
    "return SEE_OTHER" when {
      ...
      "user is signed in" in {
        ...
        forAll { (user: SignedInUser, consignee: NamedEntityWithAddress, item: GoodsItem) =>

          val request = fakeRequest.withFormUrlEncodedBody(asFormParams(consignee): _*)
          val result  = controller(user, Some(item)).onSubmit(request)

          status(result) mustBe SEE_OTHER
          redirectLocation(result) mustBe Some(routes.ConsignorController.onPageLoad().url)
        }
      }
    }
      ...
      "invalid data is submitted" in {
        ...
        forAll(arbitrary[SignedInUser], badData, arbitrary[GoodsItem]) {
          (user, consginee, item) =>
            ...
            val popForm = form.fillAndValidate(consginee)
            val request = fakeRequest.withFormUrlEncodedBody(asFormParams(consginee): _*)
            val result  = controller(user, Some(item)).onSubmit(request)
    ...
    "save data in cache" when {
      ...
      "valid data is posted" in {
        ...
        forAll { (user: SignedInUser, consignee: NamedEntityWithAddress, item: GoodsItem) =>
          ...
          val request = fakeRequest.withFormUrlEncodedBody(asFormParams(consignee): _*)
          val expectedData = item.copy(consignee = Some(consignee))
