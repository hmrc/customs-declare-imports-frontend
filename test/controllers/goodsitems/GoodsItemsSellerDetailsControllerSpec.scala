...
class GoodsItemsSellerDetailsControllerSpec extends CustomsSpec
  with PropertyChecks
  with Generators
  with OptionValues
    ...
    "return OK" when {
      ...
      "user submits valid data" in {
        ...
        forAll { (user: SignedInUser, importExportParty: ImportExportParty, goodsItem: GovernmentAgencyGoodsItem) =>
          ...
          redirectLocation(result) mustBe Some(controllers.goodsitems.routes.GoodsItemsBuyerDetailsController.onPageLoad().url)
        }
      }
    }
