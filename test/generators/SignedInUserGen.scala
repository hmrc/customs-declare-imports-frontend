...
trait SignedInUserGen {
  ...
  implicit lazy val arbitraryUser: Arbitrary[SignedInUser] = Arbitrary {
    for {
      user       <- userGen
      enrolments <- enrolmentsGen
      eori       <- eoriEnrolmentGen
    } yield {
      user.copy(enrolments = Enrolments(enrolments.enrolments - Enrolment("HMRC-CUS-ORG") + eori.enrolment))
    }
  }

  implicit lazy val arbitraryUnauthenticatedUser: Arbitrary[UnauthenticatedUser] =
    Arbitrary(userGen.map(UnauthenticatedUser))

  case class UnauthenticatedUser(user: SignedInUser)
